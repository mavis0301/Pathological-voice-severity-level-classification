...
# dir_img = Path('./CGMH/img/')
# dir_mask = Path('./CGMH/mask/')
dir_checkpoint = Path('./tmp/')

"""
訓練模型的主程式
有一些參數看起來可能會跟論文不一樣，是正常的，不同版本而已
"""
def train_model(
        model,
        device,
    ...
    for epoch in range(1, epochs + 1):
        # 最後的 20 個 epoches 不用進行 data augmentation
        if epochs - epoch <= 20:
            print("no augmentation!")
            train_loader.dataset.augment = False
        
        # 等到 global_step 大於 1000 時，改用 SGD optimizer ，解決 AdamW 在接近最小值時有收斂困難的問題
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                gpu_utilization = torch.cuda.current_stream(device).query()
...
if __name__ == '__main__':
    ...
    if not os.path.isdir("tmp"):
        os.makedirs("tmp")
    logging.basicConfig(filename = "tmp/log", level=logging.DEBUG, format='%(levelname)s: %(message)s')
